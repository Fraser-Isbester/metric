...
def test_example_metric_conforms_to_protocol():
    ...
    assert metric.computed_at is None, "Metric must not have a computed_at value before compute() is called"
    assert metric.errors == [], "Metric must not have any errors before compute() is called"
    assert metric.value is None, "Metric must not have a value before compute() is called"
